...
UINT _tmain(UINT argc, TCHAR* argv[]) {
	...
	account = CreateFile(argv[1], GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (account == INVALID_HANDLE_VALUE)
	{
	...
	SetFilePointer(account, 0, 0, FILE_BEGIN);
	...
	while (true)
	{
		FlushFileBuffers(account);
		res = ReadFile(account, &accdata, sizeof(accdata), &nRead, 0);
